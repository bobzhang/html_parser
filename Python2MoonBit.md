...
- URL proxying is a post-validation handling mode, not a way to bypass URL
  ...
  or `ht^tps:foo` before treating a value as relative; otherwise malformed
  navigations can be hidden behind the proxy rewrite. Space/control characters
  are ignored only for this scheme check, so `h ttps:foo` is still proxied as
  a relative-looking original value after validation. Python rejects empty proxy
  URLs and proxy mode without an effective proxy during object construction;
  the MoonBit constructors stay non-throwing, so sanitizer use must fail closed
...
- Rewrite-attribute transforms can have report-before-hook ordering. For
  `DropAttrs` and `AllowlistAttrs`, Python reports each unsafe attribute and
  only then runs the hook before installing the rewritten attribute map. Do not
...
- Inline `style` sanitization is a second allowlist after attribute
  ...
  delimiters, missing closing quotes, a missing `)` after a quoted or unquoted
  URL, or unexpected non-separator text after `)` should drop that declaration
  rather than trying to repair it. A later valid declaration after the
  semicolon should still survive. Normalize CSS safety checks by removing URL
...
- Some insertion modes can be approximated by a normalization pass while the
  ...
  helper-level tests should also keep detached stack shapes where the open table
  has no parent, because the foster-parenting insertion fallback is the parser
  root and reporters must no-op when the current node is already a foster target.
